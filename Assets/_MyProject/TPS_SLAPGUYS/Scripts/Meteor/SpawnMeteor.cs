...
public class SpawnMeteor : MonoBehaviour
{
    ...
    [SerializeField] List<Vector3> cubes = new();
    ...
    void SetupCubes()
    {
        ...
        foreach (var item in cubeParent.GetComponentsInChildren<Transform>())
        {
            cubes.Add(item.position);
        }
    }
    void InitializeMeteors()
    {
        for (int i = 0; i < poolSize; ++i)
        {
            ...
            if (PhotonNetwork.InRoom)
                meteor = PhotonNetwork.Instantiate(meteorPrefab.name, startPoint.position, Quaternion.identity);
            else
                meteor = Instantiate(meteorPrefab, startPoint.position, Quaternion.identity);
    ...
    Vector3 GetRandomTargetPoint()
    {
        ...
        if (cubes.Count == 0) return startPoint.position;
